...
def extract_title(docx_path: str) -> str:
    ...
    capture = False
    for _, original_text in blocks:
        text = remove_emojis(original_text)
        # Handle paragraphs that contain multiple logical lines (e.g., heading + title)
        lines = [line.strip() for line in re.split(r'[\r\n]+', text) if line.strip()]
        if not lines:
            continue

        if capture:
            return _ensure_filename_start_and_year(lines[0], filename)

        first_line = lines[0]
        if HEADER_LINE_RE.match(first_line):
            # If the title appears on the next line within the same paragraph, capture it immediately
            if len(lines) > 1:
                return _ensure_filename_start_and_year(lines[1], filename)

            inline = _inline_title(first_line)
            if inline:
                return _ensure_filename_start_and_year(inline, filename)
            capture = True
            continue

    # Fallback: some documents include the long-form title on the same paragraph as the heading.
    for _, original_text in blocks:
        text = remove_emojis(original_text)
        if 'report title' in text.lower() and 'long' in text.lower():
            lines = [line.strip() for line in re.split(r'[\r\n]+', text) if line.strip()]
            if len(lines) >= 2 and 'report title' in lines[0].lower():
                candidate = lines[1]
                if candidate:
                    return _ensure_filename_start_and_year(candidate, filename)
    ...
    priority2_candidate = None
    priority3_candidate = None
    priority4_candidate = None
    ...
    for para_idx, para in enumerate(doc.paragraphs[:50]):  # Check first 50 paragraphs
        ...
        if text_lower.startswith('the global ') or text_lower.startswith('global '):
            ...
            if full_title_match:
                ...
                if matching_keywords >= min_keywords_needed and not intro_candidate:
                    intro_candidate = extracted_title
        ...
        if (starts_with_capital and 
            ...
            if not is_heading:
                ...
                if forecast_match:
                    ...
                    if not priority2_candidate:
                        priority2_candidate = title_text
                else:
            ...
            if matching_keywords >= min_keywords_needed or 'market' in text_lower:
                ...
                if forecast_pos:
                    ...
                    if 'market' in title_part.lower():
                        if not priority3_candidate:
                            priority3_candidate = clean_text
            ...
            if starts_with_capital and not re.match(r'^\d+[\.\)]\s*', clean_text):
                ...
                if title_pattern:
                    ...
                    if len(title_text.split()) >= 3:
                        if not priority4_candidate:
                            priority4_candidate = title_text
    ...
    if priority2_candidate:
        priority2_lower = priority2_candidate.lower()
        has_segmentation_hint = (
            ' by ' in priority2_lower
            or priority2_lower.startswith('by ')
            or 'forecast' in priority2_lower
            or ';' in priority2_candidate
        )
        if has_segmentation_hint:
            return _ensure_filename_start_and_year(priority2_candidate, filename)
    if priority3_candidate:
        return _ensure_filename_start_and_year(priority3_candidate, filename)
    if priority4_candidate:
        priority4_lower = priority4_candidate.lower()
        has_priority4_hint = (
            ' by ' in priority4_lower
            or priority4_lower.startswith('by ')
            or 'forecast' in priority4_lower
            or ';' in priority4_candidate
        )
        if has_priority4_hint:
            return _ensure_filename_start_and_year(priority4_candidate, filename)
    if intro_candidate:
        return _ensure_filename_start_and_year(intro_candidate, filename)
